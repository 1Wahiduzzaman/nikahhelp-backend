...
//        }
        ...
        Log::info('file', ['data' => file_get_contents($images->file($imageName)->get())]);
        if (isset($token)) {
            $requestc = Http::withToken($token)->attach('image', file_get_contents($images->file($imageName)->getContent()))->post(config('chobi.chobi').'/api/img/'.$userUUID);
